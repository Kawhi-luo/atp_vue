...
<template id="workflow">
	<div>
        ...
        <div class="col-md-8 panel-group" id="accordion">
            <div class="panel panel-default meta_res" v-show="meta_res">
                <h5 class="panel-title">
                    <button class="btn btn-info btn-large btn-block">
                        <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne"> &#10144 Meta Result </a>
                    </button>
                </h5>
            ...
            <div class="panel panel-default font_res" v-show="font_res">
                <h5 class="panel-title">
                    <button class="btn btn-primary btn-large btn-block">
                        <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo"> &#10144 Font Result </a>
                    </button>
                </h5>
        ...
        <div class="panel panel-default link_res" v-show="link_res">
            <h5 class="panel-title">
                <button class="btn btn-info btn-large btn-block">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseThree"> &#10144 Link Result </a>
                </button>
            </h5>
        ...
        <div class="panel panel-default vpath_res" v-show="vpath_res">
            <h5 class="panel-title">
                <button class="btn btn-primary btn-large btn-block">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseFour"> &#10144 V-Path Result </a>
                </button>
            </h5>
        ...
        <div class="panel panel-default view_res" v-show="view_res">
            <h5 class="panel-title">
                <button class="btn btn-info btn-large btn-block">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseFive"> &#10144 ViewPort Result </a>
                </button>
            </h5>
        ...
        <div class="panel panel-default foot_res" v-show="foot_res">
            <h5 class="panel-title">
                <button class="btn btn-primary btn-large btn-block">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseSix"> &#10144 Footnote Result </a>
                </button>
            </h5>
        ...
        <div class="panel panel-default copy_res" v-show="copy_res">
                <h5 class="panel-title">
                    <button class="btn btn-info btn-large btn-block">
                        <a data-toggle="collapse" data-parent="#accordion" href="#collapseSeven"> &#10144 Copy Result </a>
                    </button>
                </h5>
                <div id="collapseSeven" class="panel-collapse collapse">
                    ...
                    <table id="resConwatchStyle" class="table table-striped table-bordered"></table>
                    <table id="resConwatchBand" class="table table-striped table-bordered"></table>
                    <table id="resConwatchFace" class="table table-striped table-bordered"></table>
                </div>
            </div>
        </div>
    </div>
            ...
            btnQuest(){
                ...
                if(this.url.length > 5){
                    ...
                    .then(function(data){
                                ...
                                if(checkdata[1]){
                                    for(let n in checkdata[1]){
                                        if(checkdata[1][n]){
                                            ...
                                            if (value.status === "failed") {
                                                strLinkerr += `<tr><td> <a href="${value.href}" target="_blank">${value.href}</a></td><td class='red'>${value.status}</td>
                                                        <td class='red'>${value.message}</td><td>${value.text}</td><td>${value.toWWW}</td><td class="red" >${value.statusCode}</td></tr>`;
                            ...
                            if (data.data.data.copy) {
                                ...
                                if (watchurl.indexOf("/interactive-gallery") === -1) {
                                    ...
                                    if(checkdata[2].length==0 && checkdata[3].length==0){
                                        ...
                                        if(checkdata[2].length!=0 &&checkdata[3].length==0){
                                            strCopylive += `<tr><td class='red' style="border-right:1px solid #ddd">${checkdata[2][0]?checkdata[2][0]:""}</td><td class='red'>No added content</td></tr>`;
                                            for (var x = 1; x < maxlength; x++) {
                                    ...
                                    strCopyequ += `<tr data-toggle="collapse" href="#Equivalent"><td>&diams;&#9786;&diams; Equivalent &diams;&#9786;&diams;</td></tr>`;
                                    strCopyequ += `<tbody id="Equivalent" class="collapse">`
                                    ...
                                    strwatchstyle += `<tr data-toggle='collapse' href='#strwatchstyle'><td colspan="5" bgcolor="#1e90ff" class="f1">表款</td></tr>`;
                                    strwatchstyle += `<tbody id='strwatchstyle' class='collapse' style="word-wrap: break-word;">`;
                                    for (var y = 0; y < checkdata[1].length; y++) {
                                        strwatchstyle += `<tr><td>${checkdata[1][y][0]}</td><td>aria-label:</td><td ${checkdata[1][y][2]['aria-label']==checkdata[1][y][1]['aria-label'] ? "" : "class='red'"}>${checkdata[1][y][2]['aria-label']}</td><td>${checkdata[1][y][1]['aria-label']}</td><td>${checkdata[1][y][3]['aria-label']}</td></tr>`;
                                        strwatchstyle += `<tr><td></td><td>data-band-name:</td><td ${checkdata[1][y][2]['data-band-name']==checkdata[1][y][1]['data-band-name'] ? "" : "class='red'"}>${checkdata[1][y][2]['data-band-name']}</td><td>${checkdata[1][y][1]['data-band-name']}</td><td>${checkdata[1][y][3]['data-band-name']}</td></tr>`;
                                        strwatchstyle += `<tr><td></td><td>data-casing-name:</td><td ${checkdata[1][y][2]['data-casing-name']==checkdata[1][y][1]['data-casing-name'] ? "" : "class='red'"}>${checkdata[1][y][2]['data-casing-name']}</td><td>${checkdata[1][y][1]['data-casing-name']}</td><td>${checkdata[1][y][3]['data-casing-name']}</td></tr>`;
                                        strwatchstyle += `<tr><td></td><td>data-size:</td><td ${checkdata[1][y][2]['data-size']==checkdata[1][y][1]['data-size'] ? "" : "class='red'"}>${checkdata[1][y][2]['data-size']}</td><td>${checkdata[1][y][1]['data-size']}</td><td>${checkdata[1][y][3]['data-size']}</td></tr>`;
                                        strwatchstyle += `<tr><td></td><td>data-sku-model:</td><td ${checkdata[1][y][2]['data-sku-model']==checkdata[1][y][1]['data-sku-model'] ? "" : "class='red'"}>${checkdata[1][y][2]['data-sku-model']}</td><td>${checkdata[1][y][1]['data-sku-model']}</td><td>${checkdata[1][y][3]['data-sku-model']}</td></tr>`;

                                    };
                                    strwatchstyle += `</tbody>`;

                                    strwatchband += `<tr data-toggle="collapse" href="#strwatchband"><td colspan="5" bgcolor="#1e90ff" class="f1">表带</td></tr>`;
                                    strwatchband += `<tbody id='strwatchband' class='collapse' style="word-wrap: break-word;">`
                                    ...
                                    strwatchface += `<tr data-toggle="collapse" href="#strwatchface"><td colspan="5" bgcolor="#1e90ff" class="f1">表盘</td></tr>`;
                                    strwatchface += `<tbody id="strwatchface" class="collapse" style="word-wrap: break-word;">`
                        ...
                        document.getElementById('copyequ').innerHTML = strCopyequ;
